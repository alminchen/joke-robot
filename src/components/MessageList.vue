        ...
        methods:{
            ...
            sendMessage () {
                 ...
                 this.$store.dispatch('pushMassages',
                {
                    ...
                    // {info: this.message,userid: this.userId}
                    // RobotCache.getWeather( this.message).then(data =>{
                     this.$http.get("http://api.qingyunke.com/api.php?key=free&appid=0&msg="+this.message).then(data =>{
                        console.log(data.body.content);
                         this.$store.dispatch('pushMassages',{
                            key: tool.getTimestamp(), //列表key
                            type: USER_TYPE.ROBOT, //类型
                            value: data.body.content, //内容
                            userId: '' //信息发送者
